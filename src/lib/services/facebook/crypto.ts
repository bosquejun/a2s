...
/**
 * Symmetric encryption for Facebook tokens stored in the database. Tokens are
 * encrypted at rest with AES-256-GCM using a key derived from PAYLOAD_SECRET,
 * so a database dump alone never exposes a usable Page access token.
 *
...
function getKey(): Buffer {
  ...
  if (!secret) {
    throw new Error("PAYLOAD_SECRET is required to encrypt Facebook tokens");
  }
