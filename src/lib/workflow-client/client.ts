...
export const triggerWorkflow = async <T>(
  ...
  console.log(
    `[triggerWorkflow] Starting workflow trigger - endpoint: ${endpoint}`
  );
  ...
  console.log(
    `[triggerWorkflow] Flow control settings:`,
    JSON.stringify(finalFlowControl)
  );
  ...
  try {
    ...
    console.log(
      `[triggerWorkflow] Workflow triggered successfully - runId: ${workflowRunId}, endpoint: ${endpoint}`
    );
    return workflowRunId;
  } catch (error) {
    console.error(
      `[triggerWorkflow] Failed to trigger workflow - endpoint: ${endpoint}`,
      error
    );
    throw error;
  }
