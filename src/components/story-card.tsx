...
const moodBorderClasses: Record<Mood, string> = {
  Haunting:
    "border-purple-500/20 group-hover:border-purple-500/50 bg-purple-500/5",
  Emotional:
    "border-rose-500/20 group-hover:border-rose-500/50 bg-rose-500/5",
  Confessional:
    "border-indigo-500/20 group-hover:border-indigo-500/50 bg-indigo-500/5",
  Thoughtful:
    "border-emerald-500/20 group-hover:border-emerald-500/50 bg-emerald-500/5",
  Eerie:
    "border-amber-500/20 group-hover:border-amber-500/50 bg-amber-500/5",
};

const moodTextClasses: Record<Mood, string> = {
...
const moodTagHoverClasses: Record<Mood, string> = {
  Haunting:
...
export function StoryCard({ story }: StoryCardProps) {
  const borderClass = moodBorderClasses[story.mood];
  const textClass = moodTextClasses[story.mood];
  const tagHover = moodTagHoverClasses[story.mood];

  return (
    <Link href={`/story/${story.id}`} className="block group">
      <div
        className={`relative overflow-hidden p-8 rounded-3xl border transition-all duration-700 hover:-translate-y-1 hover:shadow-2xl hover:shadow-indigo-500/10 ${
          borderClass ?? "border-slate-800 bg-slate-900/40"
        }`}
      >
        <div
          className={`absolute -right-20 -top-20 w-40 h-40 blur-[80px] opacity-0 group-hover:opacity-20 transition-opacity duration-700 rounded-full ${
            textClass.replace("text-", "bg-") ?? "bg-slate-500"
          }`}
        />
        ...
        <div className="relative z-10">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
            <div className="flex items-center gap-2">
              <span
                className={`px-3 py-1 rounded-full bg-slate-950/50 text-[10px] font-bold uppercase tracking-widest border border-white/5 ${textClass}`}
              >
                {story.category}
              </span>
          ...
          <div className="flex flex-wrap gap-2.5 mb-8">
            {story.tags.map((tag) => (
              <span
                key={tag}
                className={`flex items-center gap-1.5 text-[9px] uppercase tracking-[0.1em] font-medium text-slate-500 bg-slate-950/20 px-2.5 py-1.5 rounded-lg border border-slate-800/50 transition-all duration-300 ${tagHover}`}
              >
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-800/40">
            ...
            <div
              className={`flex items-center space-x-2 text-sm font-medium transition-all duration-300 ${textClass} opacity-0 group-hover:opacity-100 transform translate-x-4 group-hover:translate-x-0`}
            >
