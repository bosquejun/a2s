...
/**
 * Renders on the Threads Connection global. Reads the connection status from the
 * form state (no extra request) and shows either a "Connect Threads Account"
 * button (a server-side redirect into the Threads OAuth dialog) or the connected
...
export function ThreadsConnectButton() {
  ...
  const params =
    typeof window !== "undefined"
      ...
      : null;
  const error = params?.get("threadsError");
  const justConnected = params?.get("threadsConnected");
  ...
  return (
    <div style={{ marginBottom: "1.5rem" }}>
      ...
      {connected && username ? (
        ...
        <>
          <p style={{ marginBottom: "0.5rem" }}>
            Connect a Threads account to auto-post and share stories.
          </p>
          <Button
            buttonStyle="primary"
            onClick={() => window.location.assign("/api/threads/connect")}
          >
